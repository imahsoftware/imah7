...
=end
  ...
  def restableceyenvia
    ...
    @user.locked_at = nil
    @user.save(validate: false)
    mensaje = "ASEAR: Estimad@ #{@user.nombres}, nos permitimos enviar su usuario #{@user.identificacion} y contrasena #{@user.identificacion} para el ingreso a la plataforma de ASEAR, la cual sera una herramienta clave para el seguimiento y control de los colaboradores. - Url: https://appasearesp.com".html_safe
    Asearsms::SendsmsServices.new.send_sms_users(@user.id, mensaje)
